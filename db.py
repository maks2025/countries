...
async def update_or_create(conn: SAConnection, data: dict, kind: str = None) -> None:
    ...
    for code, value in data.items():
        if kind == "countries":
            values = {"code": code, "name": value}
        elif kind == "phone_codes":
            values = {"code": code, "phone_code": value}
        else:
        ...
        if r is None:
            operation = insert(country).values(**values)
        else:
            operation = (
                update(country)
                .where(country.c.code == code)
...
async def get_codes_countries_by_search_string(
    conn: SAConnection, search_string: str
) -> List[str]:
    """
